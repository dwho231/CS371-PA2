...
void *client_thread_func(void *arg) {
    ...
    send_frame.packet = "ABCDEFGHIJKMLNOP"; /* Send 16-Bytes message every time */
    send_frame.client = pthread_self();
    ...
    for (int i = 0; i < num_requests; i++){
        ...
        send(data->socket_fd, &send_frame, sizeof(send_frame), 0);
        if (epoll_wait(data->epoll_fd, events, MAX_EVENTS, 1000) <= 0){
            i--;
            continue;
        }
        tx_cnt += 1;
        recv(data->socket_fd, &recv_frame, sizeof(recv_frame), 0);
        if (recv_frame.client == send_frame.client && recv_frame.seq == send_frame.seq){
            rx_cnt += 1;
        }else{
            printf("test\n");
            i--;
...
void run_server() {
    ...
    while (1) {
        ...
        bytes = recvfrom(events[0].data.fd, &buf, sizeof(buf), 0, (struct sockaddr *)&client_addr, &client_addr_len);
        if (bytes > 0){
